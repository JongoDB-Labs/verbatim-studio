...
async def _semantic_search(
    ...
    Args:
        ...
        project_ids: Optional project IDs to scope results to.
    ...
    if project_ids:
        query = query.where(Recording.project_id.in_(project_ids))
...
async def global_search(
    ...
    if active_project_ids and not all_projects:
        recording_query = recording_query.where(Recording.project_id.in_(active_project_ids))
    ...
    if active_project_ids and not all_projects:
        segment_query = segment_query.where(Recording.project_id.in_(active_project_ids))
    ...
    if active_project_ids and not all_projects:
        document_query = document_query.where(Document.project_id.in_(active_project_ids))
    ...
    if active_project_ids and not all_projects:
        direct_doc_query = direct_doc_query.where(Document.project_id.in_(active_project_ids))
    ...
    if active_project_ids and not all_projects:
        ...
        note_query = note_query.where(
            (Note.recording_id.in_(
                select(Recording.id).where(Recording.project_id.in_(active_project_ids))
            )) |
            (Note.document_id.in_(
                select(Document.id).where(Document.project_id.in_(active_project_ids))
            ))
        )
    ...
    if active_project_ids and not all_projects:
        conversation_query = conversation_query.where(Conversation.project_id.in_(active_project_ids))
    ...
    if active_project_ids and not all_projects:
        conv_title_query = conv_title_query.where(Conversation.project_id.in_(active_project_ids))
        ...
        try:
            ...
            if remaining_slots > 0:
                segment_semantic_results = await _semantic_search(
                    ...
                    project_ids=active_project_ids if not all_projects else None,
                )
